...
if 'chart' in param and param['chart'] : st.session_state.chart = param['chart']
...
st.subheader(f"{st.session_state.chart}@{st.session_state.nodesdb}:{getpath(st.session_state.node)}", divider='rainbow')
command = f'kairos -s runchart --nodesdb {st.session_state.nodesdb} --systemdb {st.session_state.systemdb} --id {st.session_state.node} --colors {st.session_state.colors} --template {st.session_state.template} --plotorientation {st.session_state.plotorientation} --chart {st.session_state.chart} --top {st.session_state.top} --width 2000 --height 1600'
act = run(command)
if 'success' in act and act['success']:
    fig = act['data']['chart']['figure']
